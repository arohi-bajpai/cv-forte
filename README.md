...
CVForte is a powerful open-source resume builder and resume parser.

The goal of CVForte is to provide everyone with free access to a modern professional resume design and enable anyone to apply for jobs with confidence.

Official site: [https://cv-forte.com](https://cv-forte.com)
...
| **5. Successful Track Record** | CVForte users have landed interviews and offers from top companies, such as Dropbox, Google, Meta to name a few. It has been proven to work and liken by recruiters and hiring managers. |
...
CVForte is created with the NextJS web framework and follows its project structure. The source code can be found in `src/app`. There are a total of 4 page routes as shown in the table below. (Code path is relative to `src/app`)
...
5. Open your browser and visit [http://localhost:3000](http://localhost:3000) to see CVForte live

### Method 2: Docker

1. Download the repo `git clone https://github.com/xitanggg/cv-forte.git`
2. Change the directory `cd cv-forte`
3. Build the container `docker build -t cv-forte .`
4. Start the container `docker run -p 3000:3000 cv-forte`
5. Open your browser and visit [http://localhost:3000](http://localhost:3000) to see CVForte live
